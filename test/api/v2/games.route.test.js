...
describe('/api/v2/games', function () {
    ...
    describe('POST', function () {
        it('returns 201 with the location of the new resource; then we can GET the created game', async function () {
            const newGame = games.insertableGame;
            const newGameWithRelation = games.game4WithPlayersRelations;
            const resFromPost = await app()
                .post('/api/v2/games')
                .send(newGame);
            const newGameLocation = resFromPost.header.location;

            expect(resFromPost.status).to.equal(201);
            expect(isUrl(newGameLocation)).to.be.true;

            const resFromGet = await app().get(getUrlPathName(newGameLocation));
