...
const { isUrl, getUrlPathName } = require('../../test-helpers/url.helper');

describe('/api/v2/leagues', function () {
    ...
    describe('POST', function () {
        it('returns 201 with the location of the new resource; then we can GET the created league', async function () {
            const newLeague = leagues.insertableLeague;
            const resFromPost = await app()
                .post('/api/v2/leagues')
                .send(newLeague);
            const newLeagueLocation = resFromPost.header.location;

            expect(resFromPost.status).to.equal(201);
            expect(isUrl(newLeagueLocation)).to.be.true;

            const resFromGet = await app().get(getUrlPathName(newLeagueLocation));
